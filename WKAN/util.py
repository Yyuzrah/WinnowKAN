...
def FitPredModelNE(dataSection1,
    ...
    if task_type == 'classification':
        ...
        DataTra = wrap_gene_layer(tdata_rs, dataSection1.obs, "Layer")  # 分类任务使用层级标签
    elif task_type == 'regression':
        DataTra = wrap_gene_location(tdata_rs, dataSection1.obs[['x_cord', 'y_cord']])  # 回归任务使用坐标数据
    else:
    ...
    # 创建模型
    in_channels = DataTra[1][0].shape[0]
    model = FlexibleKANLIBD(in_channels=in_channels,
                            hidden_dims=hidden_dims,
                            task_type=task_type,
                            num_classes=num_classes,
                            use_relu=use_relu,
                            layer_type=layer_type)
    model = model.float().to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))

    # 训练
    CoOrg = TrainerExe()
    CoOrg.train(model=model, train_loader=t_loader, num_epochs=num_epochs, RCcountMax=number_error_try, learning_rate=initial_learning_rate)

    # 保存模型
    with open(filename3, 'wb') as filehandler:
